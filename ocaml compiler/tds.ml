...
type info =
  ...
  | InfoTypeN of string * typ
  | InfoChamp of string * typ * int * string * int
...
let string_of_info info =
  match info with
   | InfoStruct (n,t,dep,base,_) -> "Struct "^n^" : "^(string_of_type t)^" "^(string_of_int dep)^"["^base^"]"
 | InfoChamp (n,t,dep,base,_) -> "Champ "^n^" : "^(string_of_type t)^" "^(string_of_int dep)^"["^base^"]"
  | InfoConst (n,value) -> "Constante "^n^" : "^(string_of_int value)
  | InfoVar (n,t,dep,base) -> "Variable "^n^" : "^(string_of_type t)^" "^(string_of_int dep)^"["^base^"]"
  | InfoTypeN (n,t) -> "Type Nommé "^n^" : "^(string_of_type t)^"\n"
...
(* Modifie le type si c'est une InfoVar, ne fait rien sinon *)
  let modifier_type_info t i =
    ...
    |InfoChamp (n,_,dep,base,taille) -> i:= InfoChamp (n,t,dep,base,taille)
    | InfoStruct(n,_,dep,base,q) ->  i:= InfoStruct(n,t,dep,base,q) 
    |InfoTypeN (n,_) -> i:= InfoTypeN(n,t)
...
(* Modifie l'emplacement (dépl, registre) si c'est une InfoVar, ne fait rien sinon *)
 let modifier_adresse_info d b i =
     ...
     |InfoChamp (n,t,_,_,taille) -> i:= InfoChamp (n,t,d,b,taille)
     | InfoStruct(n,t,_,_,l) ->  i:= InfoStruct(n,t,d,b,l) 
