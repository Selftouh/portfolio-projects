...
let rec string_of_variable var=
(match var with
  | AstSyntax.Ident (n) -> "Ident "^n
  | AstSyntax.Var(var2) -> string_of_variable var2
  )
  ...
  let rec string_of_expression e =
    ...
    | Variable n -> (string_of_variable n)^" "
    | Booleen b -> if b then "true " else "false "
    | Entier i -> (string_of_int i)^" "
    | AllocationDynamique t -> "Allocation de type "^(string_of_type t)
    | Adresse n-> "&"^n
  ...
  let rec string_of_instruction i =
    ...
    | Affectation (n,e) ->  "Affectation  : "^(string_of_variable n)^" = "^(string_of_expression e)^"\n"
    ...
    | Typedef (n,t) -> "Typedef : "^n^" : "^(string_of_type t)^"\n" 
    | AssignationAddition (n,e) -> "AssignationAddition  : "^(string_of_variable n)^" = "^(string_of_expression e)^"\n"                    
  ...
  let string_of_typeN (TypeNomme(n,t)) =
  "Typedef : "^n^" : "^(string_of_type t)^"\n"   
  
    (* Conversion d'un programme Rat *)
  let string_of_programme (Programme (typeN, fonctions, instruction)) =
    (List.fold_right (fun t tq -> (string_of_typeN t)^tq) typeN "")^
